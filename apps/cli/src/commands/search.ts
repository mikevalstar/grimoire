...
export function registerSearchCommand(program: Command): void {
  program
    ...
    .description("Search documents using full-text keyword search (BM25)")
    ...
    .option(
      "--keyword-only",
      "Use keyword search only (default, semantic search coming in Phase 3)",
    )
    .action(
      async (
        query: string,
...
function formatSearchResults(data: unknown): string {
  const d = data as { query: string; results: Array<Record<string, unknown>>; count: number };

  if (d.results.length === 0) {
    return c.dim(`No results for "${d.query}"`);
  }

  const header = c.dim(`${d.count} result${d.count !== 1 ? "s" : ""} for "${d.query}"`) + "\n";
