...
already mirrors each reader's per-book rating from their shelves — half-star
granular, since Hardcover rates in halves — but nothing displays it.
...
that never show up — a control that lies.
...
- A book with a Hardcover edition that is **not on the reader's shelves** is
  only rated after the reader confirms: rating it adds it to their shelves as
  **Read** (`insert_user_book`, then the rating) — matching what rating means
  on hardcover.app itself. No silent shelf additions.
- A **Calibre-only** book — no Hardcover edition matched — prompts instead:
  a search of Hardcover's catalogue (their `search` API, with the reader's
  token), where picking the matching book shelves it as **Read**, rates it,
  and links it into the work as a pinned manual grouping — the same merge
  resolving a duplicate by hand performs, so the matcher never reconsiders
  it. Cancelling rates nothing; falling back to local silently would scatter
  one reader's ratings across two stores.
...
the star control must speak halves to show and set those ratings faithfully —
and local ratings adopt the same granularity rather than making the control
behave differently per source.

## Consequences

- Ratings stop being one store: reads and writes fan out on
...
- A reader's Hardcover ratings survive and live where the rest of their reading
  life is; Grimoire is a real client of their account, not a read-only mirror.
- A failed Hardcover write is a failed rating — the stars roll back, like any
  rating write failing. Offline queueing is deliberately not attempted.
- Local ratings move from whole to half stars (`REAL` column, halves enforced
  at the API boundary); existing whole-star ratings are already valid halves.
