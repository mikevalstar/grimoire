...
(TS/TSX, JSON, CSS, HTML), plus generated files that must be left alone:
...
and typescript-eslint's type-aware pass — seconds per run in a repo where
everything else is Bun-fast.
...
- **Formatter settings match the code that already existed** — 2-space indent,
  double quotes, semicolons, trailing commas, `lineWidth: 100` — so adopting it
  was a reformat, not a restyle.
- **The recommended preset, with one rule off.** `style/noNonNullAssertion` is
  disabled: `document.getElementById("root")!` and modular-arithmetic array
  indexing are deliberate here, and the rule fires on nothing else.
- **Genuine exceptions are suppressed at the line, with a reason.**
  ...
  explaining why. Unused suppressions are themselves a diagnostic, so they
  cannot rot silently.
- **Import organisation is on** as an assist action, so import blocks are
  sorted the same way everywhere.
...
- **Tailwind directives are enabled in the CSS parser** — `@theme`, `@layer`,
  `@apply` in `apps/web/src/index.css` are not valid CSS on their own.
...
monorepo runs in tens of milliseconds, so it can sit in front of every commit
without anyone minding. Formatting stops being a review topic.
...
have no Biome equivalent. If one of those turns out to catch something real,
adding ESLint alongside Biome for that narrow purpose is the escape hatch —
Biome keeps the formatting either way.
