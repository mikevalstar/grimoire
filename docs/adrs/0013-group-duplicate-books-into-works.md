...
shelf shows the same book twice, and, the part that actually breaks, rating "the
Calibre one" and rating "the Hardcover one" are two different sets of stars.
...
A decision graph is the most expressive: pairwise edges carry a verdict and a
confidence, and groups fall out as connected components. It is where manual
resolution eventually wants to live, because a "these are not the same" edge has
to survive the matcher running again. But something must then turn those
components into a group id to key anything on, which is the third option with
extra steps.
...
- Every `books` row gets its own work on insert, alone in it. Grouping is only
...
- The shelf renders works, one card per work, with the members' fields
  merged by source precedence and their `sources` unioned. That union is what
  lights up more than one mark on a book
  ([book list](../features/book-list.md)).
...
How the matcher proposes books for a work is
...
Field-level manual overrides stay unmodelled and will want a table of their own.
