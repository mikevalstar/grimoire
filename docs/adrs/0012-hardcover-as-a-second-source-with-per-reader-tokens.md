...
description: hardcover.app becomes Grimoire's second book source, reached only from the server over GraphQL, with an API token held per reader rather than per instance.
...
is that source. It is not another copy of the same thing Calibre has: Calibre
knows the files on a disk, Hardcover knows editions, covers, series and — the
part that matters — what a *person* has read, is reading, and wants to read.
...
- **It refuses to be called from a browser.** Requests must come from somewhere
  the token can be kept secret. That is not a preference of ours to weigh; it is
  their rule.
...
**Hardcover is a second source, spoken to only from `packages/api`.** The
...
**The token belongs to a reader, not to the instance** — a `hardcover_token`
column on `users`, not a `hardcover.*` preference. A Hardcover token is a
person: shelves, progress, ratings and reviews behind it are theirs, and any
write we make later happens *as them*. One instance-wide token would quietly
file a household's reading under whoever set Grimoire up, and unpicking that
afterwards would mean re-linking every account anyway.
...
that response is readable by any browser that can reach Grimoire and ADR 0008
promises nothing about who that is. Setting one is a dedicated write-only route.

**A token is stored only once it has authenticated.** Linking runs
...
**It is stored in plaintext, and that is a deliberate limit.** Grimoire has no
...
with `books` untouched until matching is designed. The UI gains no new
capability it could abuse, because it never sees a credential.
...
about not exposing a hosted instance to the internet gets sharper — that
instance can hand out `me`-shaped access to anyone who can reach it and knows a
reader id. Backups of the data directory inherit the same problem.
...
Design and behaviour are in [Hardcover connection](../features/hardcover-connection.md).
