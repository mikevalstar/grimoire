...
and this is the decisive one: shelves, reading progress, and books from other
sources (hardcover.app) all need a book record Grimoire controls, with an
identity that outlives a book being removed from Calibre. Ratings keyed on
Calibre ids already show the failure. A deleted book orphans someone's stars.

ADR 0006 established that Grimoire may keep synced copies "where a local copy
buys something real". It left open whether the UI then reads the copy instead
of the origin, and that is the decision that actually changes the architecture.

## Decision

A background sync mirrors Calibre into `grimoire.db`, and every library screen
reads from `grimoire.db`. Four parts:

**Two tables, not one.** `calibre_books` is a verbatim mirror of the connected
library; its rows appear and disappear with Calibre's. `books` is Grimoire's own
...
which sync clears when the book is absent. Connecting a second library
therefore adds books rather than overwriting them, and reconnecting the first
re-links every row.

**Covers live on disk.** Three fixed sizes, under the data dir
([ADR 0007](0007-user-data-and-asset-storage-location.md)), named by the
*Grimoire* book id. A book keeps its cover after leaving Calibre, and the
browser never learns that a Calibre id exists.

**The scheduler lives in `packages/api`.** Desktop, hosted server and
`bun dev` each get exactly one syncer no matter how many clients connect, and
all three keep speaking the same HTTP API
...
need a book record Grimoire owns can go ahead, and a second book source becomes
another writer into `books` rather than a rewrite.
...
interval, so sync status is visible UI rather than a log line. **Disk usage
grows with the library.** Three JPEGs per book plus the mirror, where before it
was zero. Sync itself is code that must handle partial failure, and the first
sync of a large library runs a long time before the app is useful.
