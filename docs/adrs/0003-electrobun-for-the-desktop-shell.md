...
needs to run that API in-process and point a webview at it. Electron would add
a second runtime (Node) and a bundled Chromium. Tauri would add Rust to a
...
The shell starts the API on 4747 and falls back to a random port, which it
hands the webview as `?apiPort=`. It loads the Vite dev server when one is
reachable and the bundled `views://mainview/index.html` otherwise.
...
host webview, so we absorb the cross-platform CSS differences ourselves.

Because the frontend only ever speaks HTTP, swapping this shell later touches
only `apps/desktop`.
