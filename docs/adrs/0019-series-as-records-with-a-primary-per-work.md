...
description: A series stops being a string on each book and becomes a row that works attach to, many at a time, with one attachment marked primary for the shelf.
...
Discworld at #6 and in Witches at #2. Both are true. Flattening that to one
...
- `series` holds the series itself: `name`, `slug`, `books_count`, and a
  nullable `hardcover_id` (UNIQUE) that is the identity when Hardcover named it.
  When Hardcover has no side of a series, `seriesKey()` identifies it: run the
  [matcher's](../features/book-matching.md) `fold()`, then strip a leading
  article and a trailing "series", "novels", "saga" and friends. Folding alone
  is not enough. "Discworld" and "Discworld Novels" differ by a word, not by
  spelling, and Calibre stores whatever the person tagging the book typed.
- `work_series` holds the attachment: `work_id`, `series_id`, `position` (REAL
  and nullable, because a series can hold a book whose place in it nobody has
  stated), `featured`, `is_primary`, and `source`. The primary key is
  `(work_id, series_id, source)`, with a partial unique index on
  `work_id WHERE is_primary = 1`.

**`source` is part of that key rather than a column beside it.** Calibre and
Hardcover routinely name the same series. With one row per `(work, series)`
pair the two sweeps would overwrite each other's provenance: the row would read
`calibre` or `hardcover` depending on which ran last, and either sweep clearing
its claim would silently take the other's with it. Each source states its own
attachment and deletes only its own rows. The read-time rule below folds them
into one series per work, keeping the best-ranked claim's provenance and taking
a position from the other if the winner has none.
...
`manual` row. [Choosing a cover](../features/book-details-panel.md) established
that rule for `works.cover_book_id`, where a human decision outranks a source
and survives every later sync. This applies it to a table instead of a column.
...
2. A `hardcover` attachment, when the instance-wide **Series from Hardcover**
   ...
   `books_count`, so the containing series beats the sub-series and a Discworld
...
one respect worth naming. Grimoire fetches About, Tags and Moods live and never
stores them, while it stores series, because sorting and grouping the shelf by
...
primary attachment's name and position, and every consumer carries on reading a
string: the shelf, sort and group, search, OPDS, the command palette. A new
...
- A book can be in two series without either being a lie, and the shelf still
  shows one.
- Grimoire now stores an answer that can contradict Calibre. It does not write
  that answer back, since Calibre stays read-only
...
- Reconcile grows a third thing to keep in step. The primary staying derived
  ...
  while Grimoire is already assembling the shelf.
...
- Folding by name will occasionally fuse two genuinely different series that
  ...
  stay two rows, since their ids tell them apart, but a Calibre string that
  folds to that name attaches to the older of them, which is a guess.
- Grimoire follows Hardcover's `canonical_id`, so a series their librarians
  merged merges here too. Nothing un-merges: if they split one later, Grimoire
  keeps the fused row until somebody attaches by hand.
