...
where it came from, your stars, the dates you read it, and the download. It also
shows the metadata neither view has room for: publisher, publication date,
...
Hardcover reading history is the exception: the panel fetches it live when a
read book's panel opens. Nothing on it writes to Calibre.
...
nowhere to go. Clicking a book has done nothing at all.
...
the shelf. It also leaves the shelf's own state untouched: scroll position,
which view, where the pointer was. A route change would not.
...
yet, so they sit under open questions rather than stubbed.
...
It sits over a scrim that dims the shelf behind it. The book's own cover,
blurred, lights the top of the panel. That is the same ambient trick
[the shell](application-shell.md) uses on the canvas, tinted here by whatever
the reader clicked.
...
- **Header.** Cover, title, authors, series and index, and the
  ...
  one. A chip that is already leading has nothing to promote, which makes it
...
- **Date read.** After Same book and before Details, for a book the current
  reader has read. The panel shows every known finish date, newest first, so
  rereads do not collapse into one date. In Hardcover mode the panel requests
  the reading history live when it opens rather than waiting for or extending
  the shelf mirror. Reduced-precision dates keep their meaning: a year stays a
  year, and a known month does not become an invented first day. The section is
  absent for unread books and for reads with no known finish date.
...
[Hardcover](hardcover-sync.md) instead of Calibre. Three switches in
[settings](settings.md) answer which ones, once and instance-wide; all three are
on by default.

Where a switch is on, the panel asks Hardcover live when it opens, with the
current reader's token, for exactly the book the panel is showing. That is the
same bargain the reading history strikes: one request for an open panel is
cheaper than widening every shelf sync, and it is current at the moment somebody
is looking. Nothing fetched this way reaches the mirror.
...
have no Calibre equivalent, so their section is absent.
...
with their logo. It opens `https://hardcover.app/books/<slug>` in a new tab,
using the slug [sync](hardcover-sync.md) already mirrors alongside every book,
and is absent for a Calibre-only book. Unlike the three switches above, it does
not depend on a linked reader account or on any preference: the slug comes from
the mirror rather than from their API, so the link is there for any matched
book.
...
the host and drops it if nobody takes it. `apps/desktop` takes it: it hands
every new-window request the webview raises to the operating system's default
browser. That keeps the link one plain anchor in `apps/web` that behaves the
same in all three delivery targets
...
shown up from them yet; if it ever does, this is the place to decide whether it
is worth a renderer.
...
The panel draws those covers as a stack of papers: the current one face up, the
others tilted behind it with a badge counting them. Clicking turns the next one
over, and that is the whole interaction. No menu, no modal, and nothing for a
book with a single cover, which is nearly all of them.
...
optimistic and otherwise silent, so the motion is the acknowledgement that the
click landed, as with [rating](rating-a-book.md). It is `motion-safe` only;
under reduced motion the covers trade places without animating.

Grimoire stores the choice on the work, for everyone: a cover is what a book
looks like, not an opinion about it, so it does not belong to a reader the way a
...
- **A member row id crosses to the browser.** Covers went out under the *work*
  id so the client never learned that member rows exist; choosing between them
  means naming them. An index into the list would have kept that secret and
  would have shifted under a sync while a panel was open.
- **The cover URL carries the chosen member.** Covers go out with a year-long
  `max-age`, so an unchanged URL would go on showing the old cover after a swap.
  The book's `coverVersion` rides along for the same reason, when the file
  itself changes ([book actions](book-actions.md)).
...
sanitizer, is a dependency and a maintenance burden this feature does not need.
...
or 404 for a book that is not a member of the work or has no cover. Both
payloads are shared Zod schemas
...
error, so the panel has something to fall back to either way. The `url` is the
...
- **Moving between books.** No next/previous, and no keyboard roving on the
  shelf behind. That is the same gap [book list](book-list.md) already records.
...
- **Reading.** A "Read now" button still needs a reader. Read status and finish
  dates are present, but there is no in-browser reading view yet.
