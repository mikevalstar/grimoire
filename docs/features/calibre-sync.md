...
running — which, on a desktop app, is whenever someone quits it.
...
Calibre's. That is what lets a book keep its cover after it is deleted from
Calibre, and it is why the browser never needs to know a Calibre id exists.
...
stop as soon as a page holds nothing newer than the watermark we stored. Ids
absent from the sweep are books that have been **deleted**, and their mirror
rows are dropped.
...
A **manual sync** ignores the watermark and does a full pass, so "it looks
wrong, re-sync" is real.
...
**Calibre's `library_id` cannot be used to detect this, and nobody should reach
for it later.** It is not an identifier Calibre generates and keeps; it is a
slug of the library folder's own name. So it fails both ways: renaming the
folder changes it though the library did not, and — the dangerous direction —
two unrelated libraries that both live in a folder called "Calibre Library"
produce byte-identical ids. There is no library uuid exposed over HTTP to fall
back on.

So identity is checked where it is actually reliable, **per book, by uuid**: if
a `calibre_id` ever reports a different `uuid` than the row we hold for it, the
id space has been reassigned, and the response is to empty the mirror and do a
full pass. Because incremental ticks only fetch *changed* books, each tick also
re-checks a small sentinel set of the lowest ids it holds, so a swapped library
whose timestamps all sit below the watermark cannot slip past. `library_id` is
still stored, as a label for diagnostics, never as identity.
...
points Grimoire at; matching on it would quietly overwrite one book's record
with another's the first time a second library was connected.

- **In the mirror, not in `books`** → insert, with `source = 'calibre'`.
- **In both** → update the mirrored fields and refresh `calibre_id`, which may
  have changed. Fields Grimoire owns are never touched by sync.
...
which is genuinely a new book as far as anything outside Calibre can tell.

Reconcile is all-or-nothing in one transaction: a half-applied pass would leave
books pointing at ids from two different libraries, which is the state this
design exists to prevent. It is skipped entirely when the mirror phase changed
nothing, so an idle library costs no writes at all.
...
Three fixed sizes, cached on disk under the data dir
([ADR 0007](../adrs/0007-user-data-and-asset-storage-location.md)) and named by
Grimoire's book id, sharded so no directory holds a hundred thousand entries:
a row thumbnail, a grid card, and a full size for the detail panel that does not
exist yet. Calibre does the scaling. A book Calibre has no cover for is marked
missing and not retried every minute; the placeholder in
[book list](book-list.md) handles it.

The whole cover tree is disposable — deleting it costs a re-sync and nothing
else. For that to be true a **full** sync checks the filesystem for every book
...
"something is happening" is never carried by movement alone.
...
detection compares — but `uuid` is `NOT NULL UNIQUE`, because it is what
...
having been keyed on `books.id` here first — so a rating can no longer outlive
its book as an orphan row keyed to a number nothing owns, *and* does not split
in two when the same book turns up from a second source. Because sync never
...
- [x] Ratings cascade with the book — keyed on `books.id` when this was written,
      on `works.id` since ([ADR 0013](../adrs/0013-group-duplicate-books-into-works.md));
...
- **First sync on a large library is a stampede**, and there is no resume: a
  first sync killed half way refetches covers it already has.
...
- **Multiple Calibre libraries at once.** Matching on uuid makes connecting a
  ...
  library — where "library" cannot be Calibre's `library_id`, for the reasons
  above. A Grimoire-minted library key is the likely answer.
