...
type: feature
title: Rating a book
description: Setting your own star rating from the shelf: hover the stars in either view and click one. Ratings are per-reader and stored in grimoire.db; Grimoire never shows Calibre's own ratings and never writes to them.
...
no one could change them. This makes them mean what they already looked like
they meant.
...
by reader and by the **work**. That is Grimoire's own idea of the book, not
Calibre's row and not a source's
...
grouped. The `X-Grimoire-User` header scopes it
...
content server refuses anonymous writes anyway. Making the ability to rate a
book depend on how the reader launched Calibre is not a feature.

### Only your own stars

Calibre's rating is not shown and is not a fallback. A book you have not rated
is blank, whatever Calibre thinks of it.
...
- **Clicking** commits, and the star bursts. The write is optimistic and
  ...
  Rating a book you liked should also feel like something. Clearing gets no
...
Local ratings use the same granularity rather than making the control behave
...
- **Hardcover** (the default for a linked reader). The stars show that
  ...
  - A book on Hardcover but not on their shelves asks first: rating it adds
    ...
    hardcover.app.
  - A shelved book that isn't finished asks too, meaning Want to Read,
    Currently Reading, or Paused. Rating it marks it **Read** with the same
  ...
  - Both ask-first flows also ask when the book was finished, the way
    Hardcover itself does: *I don't know*, *today*, a specific date, or just a
    month or year. *I don't know* is the default, and leaving the question
    unanswered means that. The answer lands on the Hardcover read entry;
    without one, Grimoire clears the entry's dates rather than letting
    Hardcover default them to today. Reduced precision goes in the way their
    own UI stores it: the first day of the period, plus
    `finished_at_precision`, an undocumented but introspectable column taking
    0 for none, 1 day, 2 month, 3 year. So "sometime in June 2023" displays as
    exactly that on hardcover.app.
  - The book itself bounds the offered dates: never after today, and never
    before the year *preceding* publication, which is one year of slack for
    advance copies. A book with no known publication date keeps the old
    open-ended range. In the finder the picked search result's release year is
    the bound, since that edition is the one being shelved.
  - A **Calibre-only** book opens a finder instead: a search of Hardcover's
    ...
    their shelves as **Read**, sets the rating, and joins the Hardcover book
    into this work as a pinned manual grouping
    ([resolving duplicates](resolving-duplicates.md)). The dialog says so.
    From then on it is one card with both marks and an ordinary Hardcover
    rating. Cancelling rates nothing; nothing falls back to local silently.
    The finder starts from a book Grimoire already has; a book it doesn't
    comes in through
...
Both views get the same control: under the series line in the grid, and in the
...
The write is optimistic. The stars change under the pointer and the request goes
out behind them, because waiting a round trip to see your own click is what
makes a rating control feel broken. A failed write rolls the stars back.

Grimoire fetches ratings once per reader as a map of book id to rating rather
than per book. Switching the current reader in [settings](settings.md)
re-fetches, so the shelf shows the new reader's ratings without a reload.
...
without it, an off-shelf book answers 409. `markRead: true` relays the
...
- [ ] A book the reader hasn't rated shows no stars, whatever Calibre's rating
      for it is.
- [ ] Grimoire writes nothing to the Calibre content server.
...
- **Sorting and filtering by rating.** "My 5-star books" is the obvious first
  filter, and it needs the rating sortable server-side. That is the same
  argument for moving the join.
