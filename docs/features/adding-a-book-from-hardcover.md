...
A **+** button beside the [header](application-shell.md) search trigger opens a
dialog that searches [Hardcover's](hardcover-connection.md) catalogue, shelves
the picked book for the current reader, and lands it in the library as a
Hardcover-sourced book. The [command palette](command-palette.md) carries a
matching entry. The reader chooses which shelf (**Want to read**, **Reading**,
**Read**) and, when it's Read, optionally when they finished it.
...
[Hardcover shelves](hardcover-sync.md). Until now the second half was read-only
in this direction: books arrived by syncing shelves the reader had built on
hardcover.app. A book they read but never owned could only enter Grimoire by
leaving Grimoire. Think a library loan, a paperback, an audiobook.
...
the one case the finder can't reach, since it starts from a book you already
have.
...
catalogue. It is the one search in the app not seeded from a book, because
...
**Which shelf.** Picking a book reveals three chips: **Want to read**,
**Reading**, **Read**. Read is the default, which is what someone reaching for
this is usually recording. Read also shows the same
...
**What lands.** Grimoire inserts the book on the reader's Hardcover shelves at
the chosen status, mirrors it, and reconciles it into the library the same way
a synced shelf entry goes. It appears as a Hardcover-sourced book, with
Hardcover's cover and Hardcover's marks, and counts toward the
...
its [details panel](book-details-panel.md) opens. The answer to "did that
work?" is the book itself.
...
than duplicating it. If Grimoire also holds a Calibre copy,
[matching](book-matching.md) takes the pair, and failing that the
[duplicates queue](resolving-duplicates.md) does, like every other pairing
nobody pinned by hand.

**When it fails.** The dialog shows Hardcover's refusal and stays open with the
pick intact, the same contract the finder has. A write that succeeded but whose
read replica lagged still adds the book; the library just learns about it on
the following sweep instead of immediately.

## API

`POST /api/users/:id/hardcover/books` shelves a catalogue book for that reader.
...
- [x] The dialog searches Hardcover's catalogue from an empty query and lets the
      reader pick one result.
...
- [x] The dialog shows Hardcover's error and stays open.
...
- Whether an add should offer a rating in the same breath, the way the
  [read-state modal](marking-a-book-read.md) does. Left out for now, since the
  stars are one click away on the card that lands.
