...
description: A multi-select dropdown on the filter bar that narrows the shelf to books from chosen sources, shown only when the library holds more than one.
...
[book list](book-list.md) toolbar. It narrows the shelf to books from chosen
...
what's on my Hardcover shelves" are both routine questions. Every book already
carries a source, drawn as a [mark](book-list.md) on each card, so the filter
groups something the reader can already see.
...
library*. It is never a fixed list, so a source nobody uses is never offered.
Sources are named and marked with their own logos, matching the marks on the
books themselves.
...
than emptying the shelf. When the reader picks a real subset, the trigger takes
the user accent and names the choice. One source gets its name, several get a
count. A narrowed shelf is never mistaken for the whole library.

A work can carry more than one source ([book matching](book-matching.md)); the
shelf shows it when *any* of its sources is selected.
...
The control hides itself, and drops any selection, while the library holds one
...
shared out of a mixed library can name sources the recipient's library has
never had. Grimoire ignores those names rather than emptying the shelf.
...
- [x] The trigger takes the accent and names the selection whenever it is not
      "all".
