...
Grimoire serves its library as an [OPDS](https://specs.opds.io/) catalog, so a
reader app — KOReader, Moon+ Reader, Thorium, KyBook — can point at a Grimoire
instance, browse the same shelf the web app shows, and download a book straight
onto the device.
...
The serving direction is the one worth building. Grimoire as an OPDS *consumer*
— a third source next to Calibre and Hardcover — is a separate feature and not
what this note is about.

Sketch, to be settled when it is actually designed:
...
- [ ] Feeds reflect Grimoire's own state — read status, per-reader shelves — not
      just Calibre's metadata.
...
- **Authentication.** OPDS clients do HTTP Basic and little else. Grimoire's
  ...
  nothing to authenticate with today — and a feed is the first thing here that
  would be exposed beyond a trusted LAN. Whose shelf a feed shows is the same
...
- **Covers.** The cached sizes on disk are sized for the shelf
