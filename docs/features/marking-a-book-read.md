...
description: Every cover gets a dog-eared corner, a check in a cut-off triangle at bottom right. It marks a book read or unread through a confirm that asks when it was finished and, optionally, what it deserved. Stored locally, or on the reader's Hardcover shelves.
...
stars. Clicking always confirms through a modal. Marking read asks when the
book was finished and offers a rating; unmarking asks whether the rating
should go too. Where the answer lives follows the reader's read-state source:
their Hardcover shelves, or Grimoire's own database.
...
read state first-class gives that question a home of its own. It also gives a
...
the reader accent and always visible. An unread book's corner appears on
hover or focus, the same rule as the stars: nothing on hover a keyboard can't
...
**Want to Read**, and the modal says so. Where the book carries a rating (and
...
- [ ] In Hardcover mode a Calibre-only book routes through the finder with
      "Add as read".
- [ ] The read-state toggle in settings is real: stored per reader, applied on
      the spot, only shown on linked cards.
...
- Unmarking on Hardcover leaves the read entry's dates alone. Deleting their
  `user_book_read` records would erase history that isn't ours. Revisit
